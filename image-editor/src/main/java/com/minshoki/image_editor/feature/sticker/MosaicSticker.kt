...
open class MosaicSticker(
    ...
    override fun setDrawable(drawable: Drawable): MosaicSticker {
    ...
    fun draw(canvas: Canvas, defaultCanvasBitmap: Bitmap, bitmapCache: HashMap<StickerView.BitmapCacheModel, WeakReference<Bitmap>>) {
        ...
        if(cacheBitmap == null) {
            ...
            try {
                val mosaic = new.mosaic()
                val circleMosaic = mosaic.getCircledBitmap(widthOffset = bound.width().toInt(), heightOffset = bound.height().toInt(), bound = bound)
    ...
    override fun setAlpha(@IntRange(from = 0, to = 255) alpha: Int): MosaicSticker {
